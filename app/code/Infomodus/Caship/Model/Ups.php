...
<RatingServiceSelectionRequest xml:lang=\"en-US\">
  ...
  <TaxInformationIndicator/>";
   ...
   <RateInformation>
	...
	<ShipTo>
      <Address>
      <AddressLine1>".substr($this->shiptoAddressLine, 0, 35)."</AddressLine1>
        ...
        if ($this->residentialAddress == 1) {
            $data .= "<ResidentialAddressIndicator>" . $this->residentialAddress . "</ResidentialAddressIndicator>";
        }
        $data .= "</Address>
    </ShipTo>
    <ShipFrom>
      <Address>
      <AddressLine1>".substr($this->shipfromAddressLine, 0, 35)."</AddressLine1>
        ...
        if (isset($xml['Response']['ResponseStatusCode']) && $xml['Response']['ResponseStatusCode'] == 1) {
            ...
            foreach ($xml['RatedShipment'] as $rated) {
                if (isset($rated['Service'])) {
                    ...
                    if(!empty($rated['RatedPackage'])){
                        if(!is_array($rated['RatedPackage'])){
                            $rated['RatedPackage'] = [$rated['RatedPackage']];
                        }

                        foreach($rated['RatedPackage'] as $ratedPackage){
                            if(!empty($ratedPackage['ItemizedCharges'])){
                                if(!is_array($ratedPackage['ItemizedCharges'])){
                                    $ratedPackage['ItemizedCharges'] = [$ratedPackage['ItemizedCharges']];
                                }

                                foreach($ratedPackage['ItemizedCharges'] as $itemCharge) {
                                    if (!empty($itemCharge['Code']) && $itemCharge['Code'] == '376') {
                                        $rates[$rateCode]["rural"] = [
                                            'price' => $itemCharge['MonetaryValue'],
                                            'currency' => $itemCharge['CurrencyCode'],
                                        ];
                                    }
                                }
                            }
    ...
    public function timeInTransit($weightSum = 0.1)
    {
        if($weightSum == 0){
            foreach ($this->packages as $pv) {
                $packweight = array_key_exists('packweight', $pv) ? $pv['packweight'] : '';
                $weight = array_key_exists('weight', $pv) ? $pv['weight'] : 0.1;
                $weightSum += round(($weight + (is_numeric($packweight = str_replace(',', '.', $packweight)) ? $packweight : 0)), 1);
            }
        }
...
<PickupDate>" . ($this->pickupDate?$this->pickupDate:date('Ymd')) . "</PickupDate>
<DocumentsOnlyIndicator />";
        ...
        if (!$curl->error) {
            $xml = $this->xml2array(simplexml_load_string($result));
            if ((isset($xml['Response']['ResponseStatusCode']) && $xml['Response']['ResponseStatusCode'] == 0)
                || (isset($xml['Response']['ResponseStatusDescription']) && $xml['Response']['ResponseStatusDescription'] != 'Success')
            ) {
                return ['error' => 1, 'response' => $xml];
            } else {
                $countDay = [];
