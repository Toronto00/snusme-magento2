...
class Config extends AbstractHelper
{
    ...
    {
        return $this->storeManagerInterface->getStore()->getBaseUrl($alias);
    }
    ...
    {
        $stores = array_keys($this->storeManagerInterface->getStores());
        foreach ($stores as $id) {
            $store = $this->storeManagerInterface->getStore($id);
    ...
    public function getProductSizes($item, $product, &$packer, $attributeWidth, $attributeHeight, $attributeLength)
    {
        ...
        if (count($children) > 0) {
            foreach ($children as $child) {
                $productChild = $this->product->getById($child->getProduct()->getId())->getData();
